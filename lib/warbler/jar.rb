...
module Warbler
  ...
  class Jar
    ...
    def run_javac(config, compiled_ruby_files)
      if config.compilation_compatibility_version
        compat_version = "--#{config.compilation_compatibility_version}"
      elsif config.webxml && config.webxml.context_params.has_key?('jruby.compat.version')
